...
<body>
	<%
	List<User> findstuList = (List<User>) session.getAttribute("findAllClassNameStu");
	User user=(User)session.getAttribute("college_major_class");
	...
	<div class="stuClass">
     ...
     <form action="<%=request.getContextPath()%>/deleteStu.action" method="post">
			...
			<table>
			<tr>
			<td><%=user.getStu_college()+"-"+user.getStu_major()+"-"+user.getStu_class()%></td>
			</tr>
			</table>
			</center>
		</div>
		...
		<table class="table table-bordered" id="tab">
			...
			<tbody>
				...
				<!--可以加一行，在前端输出int型变量 -->
					...
					<td><%=findstuList.get(i).getStu_card()%></td>
...
<div class="modal fade" id="myModal" tabindex="-1" role="dialog" aria-labelledby="myModalLabel" aria-hidden="true">
	<div class="modal-dialog">
		...
		<form name="myForm" action="<%=request.getContextPath()%>/updateStu.action">
				<center>
